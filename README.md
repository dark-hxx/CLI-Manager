...
- **Reliable live status** - SSH sessions with a validated installed Hook reuse one per-host protocol 1.5 Agent bridge with bounded handshake/response timeouts, heartbeat, reconnect gates, and a streaming Host/client-isolated spool
...
- **Read-only remote files** - browse a confined lazy tree, search names or text, preview UTF-8 text and images, and copy remote paths without passing them to local filesystem or Git APIs

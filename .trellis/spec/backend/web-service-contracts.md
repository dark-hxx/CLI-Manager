...
| POST | `/api/operations` | Creates/idempotently returns a conversation operation |
...
```text
submitted -> waiting_device | accepted
waiting_device -> accepted
accepted -> running
running -> succeeded | failed | rejected
non-terminal -> canceled | timed_out
```

- Current Web release accepts only `conversation.start` and `conversation.prompt`, both with non-empty `payload.prompt`.
...
- Validation rejection still follows the persisted state machine: `accepted -> running -> rejected`; do not send an unsupported `accepted -> rejected` jump.
...
storage.upsert_device_hello(...).await?;
// Terminal status is accepted only after the persisted state reached Running.
storage.update_operation_status(device_id, id, OperationStatus::Succeeded, ...).await?;
```

...
4. SSH Host 提供可选的远端附件目录配置；配置为空时使用 Agent 默认目录，配置后图片/文件写入该 Host 的自定义目录下，并继续使用会话/上传 ID 隔离，不能写入其他 Host 或其他会话。
5. 自定义目录只允许安全的远端 POSIX 路径（建议支持绝对路径与 `~/...`，拒绝相对路径、`..`、控制字符、反斜杠和路径逃逸）；Agent 只管理自己创建的附件子目录，不能清理或覆盖用户已有的无关文件。
...
10. 每个 SSH Host 提供独立的附件上传入口。上传面板采用双栏文件传输布局：左侧选择本地文件，右侧展示该 Host 的 Agent 附件目录，底部展示排队、上传中、成功和失败状态。
...
| Host 附件面板 | 无 Agent、Agent 能力过旧、默认/自定义目录、目录不可用、本地取消选择、多文件/超限/失败/成功、两个 Host 同时操作均有明确状态且不串传 |
...
- [ ] SSH Host 可保存自定义远端附件目录；空配置使用 Agent 默认缓存目录，非空配置写入隔离的 Agent 附件子目录并返回可被远端 CLI 读取的路径。
...
- [ ] 从 Host 列表打开附件面板后，可以选择一个或多个本地文件并上传到该 Host 的 Agent 附件目录；右侧远程列表和底部传输队列随状态刷新。

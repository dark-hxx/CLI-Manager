...
const PROJECT_SELECT = "SELECT id, name, path, group_id, sort_order, cli_tool, cli_args, startup_cmd, env_vars, shell, provider_overrides, worktree_strategy, worktree_root, worktree_deps_prompt_enabled, created_at, updated_at FROM projects ORDER BY sort_order";
...
async function replaceWorkspace(db: Awaited<ReturnType<typeof getDb>>, workspace: WorkspaceBackup) {
  ...
  await batchInsert(db, "projects", ["id", "name", "path", "group_id", "sort_order", "cli_tool", "cli_args", "startup_cmd", "env_vars", "shell", "provider_overrides", "worktree_strategy", "worktree_root", "worktree_deps_prompt_enabled", "created_at", "updated_at"], projects, (item) => [
    ...
    item.env_vars ?? "{}", item.shell ?? "", item.provider_overrides ?? "{}", item.worktree_strategy ?? "disabled",
    item.worktree_root ?? "", integerOr(item.worktree_deps_prompt_enabled, 0), item.created_at ?? now, item.updated_at ?? now,
  ]);

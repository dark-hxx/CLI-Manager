...
- 全部手写代码完成 2000 物理行上限与单行 500 字符治理，迁移基线清空；规范要求按职责拆分，禁止压缩代码、数字分片或新增豁免。
- 前端整体迁入 app/features/shared，跨功能使用独立 api 模块或既有 index/state 入口；全局偏好、UI、平台适配和纯数据契约归共享层。清理 7 个冗余旧入口，逐项验证 511 个模块及 2106 条导入关系不变。
- XTerm、TerminalTabs、Hook 设置、历史 Store 和项目侧栏拆出控制、视图、模型与专项辅助职责；保留 Hook 顺序、懒加载、回调依赖、PTY 生命周期及完整 JSX。
- terminalStore 保留唯一 Store 和运行时状态所有者；动作工厂共用同一 Zustand API，计时器、序号、串行保存及心跳不重复创建，并新增运行时回归测试。
- 文件编辑器拆出 Markdown 导航、控制 Hook、视图与模型，恢复既有 300 行职责上限；保留请求身份、脏文件确认与固定 Diff 组合边界，新增异步导航回归。
- Rust 历史、cc-connect、Hook、Git、provider、daemon、SSH、状态栏等按职责拆分；236 个源文件归入 features/infrastructure/shared，通过 lib/commands 显式路径保持原 crate 命名空间和可见性，243 个完整源文件比对仅改变模块/资源路径。
- 翻译拆为 14 个领域的中英模块，运行时入口为 `src/shared/i18n/index.ts`；组件 CSS 拆为 16 个有序模块。4472 对翻译键值及 1248 个 CSS 顶层规则逐项保持不变；压缩 JSX 和 Rust 长字符串展开后校验编译语义与字节。
- 新增独立 `check:architecture` / `report:architecture`，检查长度、前端公共入口及可解析的 Rust 分层依赖，报告字节、粗略 Token 和导入数；不新增依赖、不绑定构建或启动。测试跟随实际源码位置，并清理引用已删除光标函数的遗留测试。

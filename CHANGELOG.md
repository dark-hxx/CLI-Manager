...
- Agent bridge 协议升级到 `1.5`；SSH 项目文件面板新增只读远端浏览，通过复用 bridge 懒加载目录、搜索文件名或文本并预览 UTF-8 文本与图片。Agent 对根目录、相对路径、symlink、遍历深度、目录/搜索结果和单文件大小实施限制，UI/store 同时拒绝保存、创建、重命名、删除、移动、拖拽、本地 Explorer/Finder、Git 与 Worktree 操作。

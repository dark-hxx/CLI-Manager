...
- 前端实现整体迁入 app/features/shared：跨功能模块通过独立 api 模块或既有 index/state 入口访问，全局偏好与纯数据契约归共享层；清理 7 个冗余旧入口，保留原始模块加载关系、CSS 内容及窗口懒加载。更新源码契约测试位置，并移除引用已删除光标函数的历史遗留测试。
- 文件编辑器按控制 Hook、Markdown 导航、完整视图与模型拆入 files 功能目录；保留请求身份、Hook 顺序、脏文件关闭确认与 Diff 组合边界，修复既有 300 行职责上限测试，并新增导航异步回归测试。
- 展开 7 个组件的压缩 JSX 与 2 处 Rust 字符串表达式，逐项核对编译后 React/JavaScript 结构和字符串字节不变；严格架构检查已达到零超长文件、零超长行，完整目录归属迁移继续进行。
- terminalStore 拆成唯一 Store、共享同一 Zustand API 的运行时动作工厂、类型与启动/状态/布局/子代理辅助模块；保留计时器、序号、串行保存和心跳的单一所有者。新增运行时回归测试，全部手写代码文件已低于 2000 行；超长行及完整目录依赖治理继续进行。
- TerminalTabs 按标签/悬浮卡/分屏/拖拽/对话框拆分，控制 Hook、工具栏渲染和作用域空状态分别定位；保留懒加载、回调依赖和完整视图。终端组件共 512 个声明/语句逐项比对不变（仅补充主题联合类型标注），当前仅 terminalStore 仍超 2000 行。
- XTerm 按控制 Hook、视图、链接与诊断辅助职责拆分，原生命周期语句与 JSX 保持不变；更新过期的快照恢复源码断言，补测快照分支在尺寸适配后修复光标再释放输出的顺序。
- Hook 设置页、历史 Store 与项目侧栏迁入功能目录，按控件/模型、规范化/请求/缓存/元数据、状态控制/视图/删除确认拆分；保留旧导入入口、唯一 Store 和原 Hook 顺序。逐项验证 375 个声明/语句及完整侧栏 JSX、删除确认逻辑不变，剩余 3 个终端相关超限文件继续治理。
- 历史服务和 cc-connect 按解析、目录、统计、配置、启动与凭据等职责继续拆分，保留原 Tauri 命令及类型入口；历史/cc-connect 测试按领域分组，源码契约测试改读实际职责文件。全部 Rust 文件已低于 2000 行，剩余 6 个前端超限文件继续治理。
- Rust 基础层按职责拆出数据库迁移、状态栏主题、Git 辅助逻辑、daemon 回放/传输/事件、Hook 配置、历史目录查询/同步/索引写入及独立测试模块，保留原命令入口和数据协议；当前超 2000 行文件缩减至 8 个，剩余重构继续进行。
- 新增独立 `check:architecture` / `report:architecture`，检查 2000 行上限、超长行和新分层依赖，提供文件字节与粗略 Token 规模报告；不增加依赖、不绑定构建或启动。
- 项目规范明确前端 app/features/shared、Rust 分层和按职责拆分规则；迁移期间保留只缩减的历史基线，严格模式用于最终零超限验收。
- 首批将翻译字典拆为 14 个领域的中英模块，保留 `src/lib/i18n.ts` 运行时入口；将组件样式拆为 16 个按原序导入的职责模块。逐项验证 4472 对翻译键值和 1248 个 CSS 顶层规则不变，超限文件从 25 个减至 23 个。

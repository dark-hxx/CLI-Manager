...
- **终端文件引用原子化输入**：包含文件引用的活动输入行改由镜像层统一渲染，文件标签按自身宽度实际占位，不再依赖额外空格；原始 `@path` 仍同步保留在 PTY 中，视觉光标会停在标签前后，方向键、点击和删除按整个文件引用移动或处理，跨越标签时输入行不再因原生光标逐格同步而晃动，整段删除引用时也只发生一次布局收缩。

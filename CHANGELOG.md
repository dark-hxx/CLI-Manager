...
### Fluxion AI Token 中转站与默认供应商

- 侧边栏新增带呼吸动画的 AI Token 中转站入口，展开与折叠布局均支持键盘访问；点击后打开设置中的“赞助商”页，动画在减少动效偏好下自动停用。
- 新增 Fluxion AI 合作伙伴卡片，展示统一接入全球主流 AI 模型的平台介绍、兑换码 `CLIMANAGER` 福利、本地横幅/Logo 素材和固定注册入口；注册链接打开失败时给出本地化提示。
- Provider 数据库初始化幂等补齐 Claude、Codex、Grok Build 三个 Fluxion 内置供应商，预填对应 API 基础地址与协议配置，API Key 保持为空并将内置项置于初始列表首位；重复启动或已有自定义记录不会覆盖用户排序、当前项和密钥。
- Fluxion 供应商编辑表单的 API Key 区增加注册入口及福利说明，所有跳转统一使用 `https://fluxionai.space/register?source=github&campaign=climanager`。
- 优化赞助商卡片视觉层级：横幅按原比例完整展示，Logo 提高清晰度，兑换码突出显示，CTA 在窄屏下自适应整行，并兼容深色主题。
- 进一步裁切横幅素材内嵌的左右边线，并以轻量渐变、层次阴影和更精细的按钮/福利卡片边界提升整体质感。
- 移除赞助商卡片 Logo 外层边框，避免素材周围出现额外描边。
- 统一赞助商卡片福利区域与注册按钮的高度，保持操作区视觉对齐。
- 强制注册按钮填满操作区网格行高度，修正 Mantine 默认按钮高度导致的上下不齐。
- 移除福利区域边框并缩小注册按钮为紧凑尺寸，减少操作区视觉负担。
- 将注册按钮纳入福利操作区的统一阴影背景，强化整体容器的一致性。
- 调整侧边栏底部工具栏顺序，将 AI Token 中转站入口置于统计入口之前；设置导航中的“赞助商”移动至“关于”上方。
- Fluxion AI 默认供应商卡片新增“赞助商”徽章及平台介绍，帮助用户识别合作伙伴供应商。
- Fluxion AI 供应商的“打开官网”按钮统一使用带来源参数的赞助注册链接，并提升按钮视觉权重；卡片补充 CLIMANAGER 兑换码福利说明。
- 兑换码 `CLIMANAGER` 在 Fluxion 供应商介绍中使用独立主题色徽章样式突出显示。
- 双击 Fluxion 兑换码会复制代码并提示，3 秒后自动打开带来源参数的注册链接；修复供应商“打开官网”按钮无响应问题。

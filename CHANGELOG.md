...
- **文件拖拽跟手性优化**：拖拽预览的位置更新改为逐帧直接更新 DOM 位移，不再在每次鼠标移动时重渲染整棵文件树，改善文件行悬浮反馈延迟和拖入终端输入框时图标滞后的问题。
...
- **终端文件引用原子化输入**：包含文件引用的活动输入行改由镜像层统一渲染，文件标签按自身宽度实际占位，不再依赖额外空格；原始 `@path` 仍同步保留在 PTY 中，视觉光标会停在标签前后，方向键、点击和删除按整个文件引用移动或处理，跨越标签时输入行不再因原生光标逐格同步而晃动，整段删除引用时也只发生一次布局收缩；多个长路径触发软换行或 TUI 视觉换行后仍保持紧凑标签布局。
